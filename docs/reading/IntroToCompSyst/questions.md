...
b. Результаты двух сумматоров (w + x; y + z) — это m, n. Поступают в сумматор, результат (w + x + y + z) — это m. Поступает в умножитель, где n = 1/4.
...
ISA описывает то, *что* процессор делает (набор команд, регистры, типы данных). Микроархитектура — это реализация ISA, определяет, *как именно* выполняются команды.
...
Да, потому что каждый уровень выше облегчает нам работу в целом, но при необходимости можно спускаться ниже. Тогда каждый уровень объяснит нам более подробно работу чего-либо. 

Абстрагируясь от подробностей, мы быстрее и легче работаем с чем-либо, но при возникновении проблем необходимо уметь глубже рассматривать вопрос.
...
Программное обеспечение, которое покупается в магазине (например, Microsoft Word), распространяется в форме машинного кода — в виде ISA того компьютера, на котором оно будет выполняться. 

Исходный код на высокоуровневом языке (C++, C# и т. д.) компилируется в исполняемый файл, содержащий инструкции ISA. Пользователь не получает исходный код; он получает готовую программу, которую процессор может выполнять напрямую. 
...
ISA не меняется между поколениями микроархитектур, чтобы обеспечить обратную совместимость (backward compatibility). 

Когда пользователь обновляет компьютер или покупает новый процессор, он ожидает, что всё его существующее программное обеспечение будет работать без изменений. Если бы Intel изменила ISA между Pentium II и Pentium III, старые программы (скомпилированные под старую ISA) не смогли бы выполняться на новых процессорах. Это разрушило бы экосистему и заставило бы пользователей покупать новые версии всех программ. 

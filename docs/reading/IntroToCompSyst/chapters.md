...
- определённость — чёткое определение шагов (например, смешивать до однородности — непонятно, где начинается однородность);
- эффективная вычислимость — решение может быть получено за конечное число шагов.
- предельность — процедура рано или поздно закончится.
...
- opcode (код операции) — *что* сделать;
- operand (операнд) — *над чем* сделать.
...
2. Механизмы, которые компьютер использует, чтобы найти, *где* находятся операнды — режим адресации (addressing mode).

> Разные ISA содержат разные опкоды, типы данных и режимы адресации.

Перевод с высокоуровневого языка (например, C) на ISA (например, x86) производится с помощью *компилятора* (C to x86 compiler).

Перевод с уникального языка ассемблера на ISA — с помощью *ассемблера*.
